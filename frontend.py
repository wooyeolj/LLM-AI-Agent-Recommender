...
with st.sidebar:
    st.header("AI 추천 시스템")
    st.markdown("GitHub: [wooyeolj/LLM-AI-Agent-Recommender](https://github.com/wooyeolj/LLM-AI-Agent-Recommender)")
    st.divider()
...
def render_table(category: str, data: list):   # 백엔드에서 받은 추천 결과 리스트를 테이블로
    if not data:
        return
    ...
    if category == "MODEL":
        ...
        for i, item in enumerate(data, 1):
            rows.append({
                ...
                "최근 업데이트": item.get("last_updated", "-") or "-",
                "링크": item.get("url", ""),
            })
        df = pd.DataFrame(rows)
        st.dataframe(
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        if message.get("table_data"):
            render_table(message.get("category", ""), message["table_data"])
        ...
        try:
            with requests.post(
                ...
                for raw_line in resp.iter_lines(decode_unicode=True):
                    ...
                    if etype == "status":
                        step = event.get("step", "")
                        status_ph.info(f"[...] [{step}/4] {event.get('message', '')}")
                        ...
                        with table_ph.container():
                            ...
                            render_table(category, table_data)
